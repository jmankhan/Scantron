...
public class ScanRow {
	...
	private void generateBubbles(BufferedImage testImage) {
		...
		for(int x = this.x; x < (this.width+this.x); x += bubbleWidth) {
	...
	public int getRowAnswer() {
		...
		for(int i=0; i < bubbles.size(); i++) {
			...
			if(bubbles.get(i).getAverageColorDensity() > highest) {
				answer = i;
				highest = bubbles.get(i).getAverageColorDensity();
			}
		}
