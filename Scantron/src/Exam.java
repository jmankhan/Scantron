	...
	public void grade() {
		...
		for(StudentTest test : tests) {
			...
			for(int i=0; i<answerRows.size(); i++) {
				if(answerRows.get(i).getRowAnswer() - testRows.get(i).getRowAnswer() > 0) {
					//print diff location
					System.out.println("found error at row " + i + ", form " + test.getName() + ". Correct answer was " + 
					(answerRows.get(i).getRowAnswer()) + ", student answered " + (testRows.get(i).getRowAnswer()));
				}
			}
		}
	}
